...
with member x.Market = match x with (EquityTrade(_, _, _, _, m, _, _) 
                       | FixedIncomeTrade(_, _, _, _, m, _, _)) -> m
     member x.Principal = match x with (EquityTrade(_, _, _, _, _, q, u) 
                          | FixedIncomeTrade(_, _, _, _, _, q, u)) -> q * u
